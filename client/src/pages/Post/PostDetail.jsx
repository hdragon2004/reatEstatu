...
const PostDetail = () => {
  ...
  // Hàm mở Google Maps với vị trí của bài post
  const handleOpenGoogleMaps = () => {
    // Ưu tiên sử dụng tọa độ nếu có (chính xác hơn)
    ...
    if (lat && lng) {
      // Mở với tọa độ (chính xác nhất)
      googleMapsUrl = `https://www.google.com/maps?q=${lat},${lng}`;
    } else {
      // Nếu không có tọa độ, sử dụng địa chỉ
      // Ưu tiên: FullAddress > CityName/DistrictName/WardName > area nested data
      const address = fullAddressForMap || 
        (post.fullAddress) ||
        (post.cityName || post.districtName || post.wardName 
          ? `${post.street_Name || ''}, ${post.wardName || ''}, ${post.districtName || ''}, ${post.cityName || ''}`.replace(/,\s*,/g, ',').replace(/^,\s*|,\s*$/g, '')
          : `${post.street_Name}, ${post.area?.ward?.name || ''}, ${post.area?.district?.name || ''}, ${post.area?.city?.name || ''}`.replace(/,\s*,/g, ',').replace(/^,\s*|,\s*$/g, ''));
      
      if (address) {
        // Mở với địa chỉ
        googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`;
      } else {
        alert('Không có thông tin địa chỉ hoặc tọa độ để hiển thị trên bản đồ');
        return;
      }
    }
      ...
      <div className="post-header">
        <div className="post-content">
          ...
          <div className="post-meta">
            <div className="post-meta-item clickable-address" style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <div onClick={handleAddressClick} style={{ display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', flex: 1 }}>
                <FaMapMarkerAlt />
                <span>
